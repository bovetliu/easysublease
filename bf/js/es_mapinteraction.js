...
var MAP_RENDER = Backbone.Model.extend({
	...
	set_status : function (id, status_num, entry) {
		...
		this.get('model').set('to_be_set_expired', id); // this => EasySubOrg.comm_unit.getAfterSettingExpired
	...
	renderRentalResults : function () {  //once the model has change at rental_search_result, this will be called
		...
		//console.log("entered renderRentalResults:");
		temp_result_array = this.get('model').get('rental_search_result');
		//console.log( temp_result_array[0]);
		
		if (temp_result_array ) {
			temp_result_array.forEach( function ( one_entry, index, ar){
				var temp_latLng = new google.maps.LatLng(one_entry[1], one_entry[2]);
				//ar[index] is an entry returned from SERVER
				//addMarker : function(location, cat,           url_str,        memo,             id, draggable)
				ClassRef.addMarker( temp_latLng, ar[index][3], ar[index][4], ar[index][5], ar[index][0]  );	
			});		
		}
	},
	...
	*/
	renderRideResults: function(){
		this.clearMapAddons();
		var search_result = this.get('model').get("travel_search_result");
  ...
  // [END snippet-style]	
	...
	var listener_data_right_click = temp_map.data.addListener('rightclick', function(event) {
		...
		if ( event.feature.getProperty('route_class') != "college_shuttle"){
			...
			EasySubOrg.MAP.render_01.addMarker(event.latLng,3);
		}
	});	
...
MapCommonCalc.prototype.buildGraph = function (  ) {
	...
	MapCCHref = this;
	console.log( "how many proparray"+this._proparray.length  );
	MapCCHref.headNodeArray = [];
...
MapCommonCalc.prototype.createFeature = function(prop_array) {
	...
	MapCCHref = this;debug =false;
	ne = new google.maps.LatLng(30.364225, -95.965573);
	sw  = new google.maps.LatLng(30.276502, -96.125561 );
	debugBounds = new google.maps.LatLngBounds(  sw,ne); 
	var color;
	...
	if (prop_array['encoded_polyline'] != "void") {  // new feature of database
		...
		if (debug) {	
			for (var i = 0; i <prop_array['latlngs'].length ; i ++ ) {
					if (  debugBounds.contains(prop_array['latlngs'][i]  ) ){
						temp = new google.maps.Marker({
							map:EasySubOrg.MAP.cu_01.get('map'),
							draggable:true,
							animation: google.maps.Animation.DROP,
...
MapCommonCalc.prototype.focusOn = function (encodedPath, width_ratio, height_ratio, offset_ratio, focusID) {   
