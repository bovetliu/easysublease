...
*/
  ...
  // [END snippet-style]
	...
	/*tb plase*/
		var A1 = new google.maps.LatLng(30.655178, -96.275625);
	var A2 = new google.maps.LatLng(30.654920, -96.275175);
	...
	var B2  = new google.maps.LatLng(30.6548,-96.27497000000001 );
...
SegmentNode.prototype.getGeoLength = function( latLngA, latLngB) {
	...
	this.isLongSeg = (Math.sqrt( x_1 * x_1 + y_1 * y_1)>0.00170) ? true:false;
...
SegmentNode.prototype.distanceToOrigin = function( isLongSeg ) {
	...
	if (isLongSeg) {
		...
		this.dist = Math.sqrt( x_1 * x_1 + y_1 * y_1);
	}else {
		...
		this.dist = parseInt(temp_dist.toString().split('.')[1].slice(0,5));
	}
}
...
SegmentNode.prototype.buildBounds  = function(  dir ) {
	SNhref =this;
	if (Math.abs(dir) > 67.5) {
		offset_lat = 0.00015; offset_lng = 0; 		
	} else if (Math.abs(dir) <= 67.5 && Math.abs(dir) > 22.5 )  {
...
SegmentNode.prototype.dominatedBy = function ( incomingNode) {
	...
	if (this.getGeoLength( cankao1, cankao2) > 0.35 * this.geoLength)  // overlapping length larger than 35% of total length of this line segment
	return true;
	else return false;  
} 
...
MapCommonCalc.prototype.disperseExistenceOf  = function( segNode) {
	...
	var dist = segNode.dist;   var dir = (segNode.isLongSeg) ?  segNode.dir : Math.round(segNode.dir / 10 );
  var checked_nodes = [];
	if (segNode.isLongSeg) {
		for ( var i = dir -2; i <= dir +2 ; i ++) {
			for (var j = dist -2; j <= dist +2; j ++) {
				//console.log([i,j]);
				if (this.checkAndOpen(i,j, this.long_Node_Hashmap)) { 
						this.long_Node_Hashmap[i][j].push(segNode);	
				}		
				else {  // someone came here first situation 
					//console.log("else");
					for (var k = 0; k <this.long_Node_Hashmap[i][j].length; k ++ ){
						if ( checked_nodes.indexOf( this.long_Node_Hashmap[i][j][k] ) >= 0 ) {
							continue;						
						}
						else{   // not checked nodes
							if (this.long_Node_Hashmap[i][j][k].isAlignWith(segNode) ){
								this.long_Node_Hashmap[i][j][k].shareWith (segNode)
							}
							checked_nodes.push( this.long_Node_Hashmap[i][j][k] );
						}
					}
					this.long_Node_Hashmap[i][j].push(segNode); // eventually, put the segNode into this virtual 2D array
				}// someone came here first situation : else 
			}	// for loop of j
		}//for loop of i
	} else{
			for ( var i = dist -2; i <= dist +2 ; i ++) { // one special representation of center node to origin
				for (var j = dir -1; j <= dir +1; j ++) {    // dir * 10
 					//console.log([i,j]);
					if (this.checkAndOpen(i,j, this.short_Node_Hashmap)) { 
							this.short_Node_Hashmap[i][j].push(segNode);	
					}		
					else {  // someone came here first situation 
						//console.log("else");
						for (var k = 0; k <this.short_Node_Hashmap[i][j].length; k ++ ){
							if ( checked_nodes.indexOf( this.short_Node_Hashmap[i][j][k] ) >= 0 ) {
								continue;						
							}
							else{   // not checked nodes
								if (this.short_Node_Hashmap[i][j][k].isAlignWith(segNode) ){
									this.short_Node_Hashmap[i][j][k].shareWith (segNode)
								}
								checked_nodes.push( this.short_Node_Hashmap[i][j][k] );
							}
						}
						this.short_Node_Hashmap[i][j].push(segNode); // eventually, put the segNode into this virtual 2D array
					}// someone came here first situation : else 
				}	// for loop of j
			}//for loop of i
