...
$(document).ready(function mainsite_ready_logic(){
  ...
  EasySubOrg.Filter.listing = new (Backbone.Epoxy.View.extend({
    ...
    setBoundaryFilter:function( googleBounds){
      var ClassRef = this;
      var ne = googleBounds.getNorthEast();      // for this one, Mapbox and google is the same
      var sw = googleBounds.getSouthWest();      // for this one, Mapbox and google is the same

      if (mapcover.model.get("map_vender") === "mapbox"){
    ...
    updateBoundaryFilter:function(){
      ...
      ClassRef.setBoundaryFilter(map.getBounds());   // Google & Mapbox the same
    },
    //[general purpose method]
    ...
    initialize:function(){
      ...
      this.model.on(watchlist.join(" "), function behaviorChangeHandler (){
        ...
        EasySubOrg.comm_unit.requestData("/data_api/listing/conditional", query, function success(data){
          // expecting an array
          // console.log(data);
          EasySubOrg.mapmng.set("listing_data", data);  // mapmng is on listing_data
        });
      });
      ...
      if (mapcover.model.get("map_vender") === "google"){
      ...
      } else if (mapcover.model.get("map_vender") === "mapbox"){
        setTimeout(function(){
          EasySubOrg.comm_unit.requestData("/data_api/listing/conditional", initial_query, function success(data){
            EasySubOrg.mapmng.set("listing_data", data);
          });
    ...
    }//instance EasySubOrg.Filter.listing initialize() ends
  ...
  EasySubOrg.mapmng = new ( Backbone.Model.extend({
    ...
    renderData:function(){
      ...
      var temp_marker_option = {
        ...
        latLng: L.latLng(-33.397, 150.644),// mapcover.model.get("mc_map_events")['rightclick'].latLng,   //google/mapbox
        map: mapcover.model.get("map")
      }
      ...
      _.each( ClassRef.get("listing_data"), function(element, index, arr){
        ...
        var tempLatLng = L.latLng( element.unit_traits.lat, element.unit_traits.lng);  //google/mapbox
